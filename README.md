...
| Minecraft UI + schematic workspace | `8093` | ✅ after release | Always-on UI; anonymous browsing, administrator-only library curation. Deployment is pending the release gate |
...
- **Release the Minecraft UI on `:8093`.** Its application, private schematic
  ...
  first pin an immutable viewer image digest, create the three file secrets, and
  approve the two-service cutover. The landing-page link remains gated on its
  server-side TCP probe until then.
